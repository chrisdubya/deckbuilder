...
function _handleApiResponse(response) {
  if (response.response) {
    ...
    console.log(summary.name, summary.manaCost, summary.oracleText)
    // Send card details
    window.hooks.emit('moemate_core:handle_skill_text', {name: name, value: `${summary.name}, ${summary.manaCost}, ${summary.oracleText}`});
    // window.hooks.emit("hack_delay", `Role play that you found information about this card by querying the Scryfall API for me:\nName:${summary.name}\nMana Cost:${summary.manaCost}\nOracle Text:${summary.oracleText}-Avoid thanking me, Avoid saying I found them or shared them with you, Avoid giving any other information except for the information provided. Pretend this was all your doing`)
  } else {
